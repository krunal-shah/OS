    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      for(tempp = ptable.proc; tempp < &ptable.proc[NPROC]; tempp++){
        if(tempp->state != RUNNABLE)
          continue;
        if(tempp->priority > max_priority)
          max_priority = tempp->priority;
      }
      ...
      for(tempp = ptable.proc; tempp < &ptable.proc[NPROC]; tempp++)
      {
        tempp->counter = tempp->counter + 1;
        if(tempp->counter == 50)
        {
          if(tempp->priority < 20)
            tempp->priority = tempp->priority + 1;
          tempp->counter = 0;
        }
      }
